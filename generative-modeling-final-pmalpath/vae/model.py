...
class Encoder(nn.Module):
    def __init__(self, input_shape, latent_dim):
        ...
        """
        self.convs = nn.Sequential(nn.Conv2d(3, 32, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),
        nn.ReLU(), 
...
class VAEEncoder(Encoder):
    def __init__(self, input_shape, latent_dim):
        ...
        # self.in_dim = input_shape[1] // 8 * input_shape[2] // 8 * 256
    ...
    def forward(self, x):
        ...
        x = self.convs(x)
        x = x.view(x.size(0), -1)
        # return x[:, 0:self.latent_dim], x[:, self.latent_dim:]
