...
log_wandb = True
if (log_wandb):
    wandb.init(project="VLR_Homework_2")
def ae_loss(model, x):
    """ 
    ...
    l = model.encoder(x)
    # print(l.size())
    recon_l = model.decoder(l)
    # print(recon_l.size())
    loss = loss(recon_l, x)
    loss = loss.reshape(x.shape[0], -1).sum(dim = 1).mean()
...
def vae_loss(model, x, beta = 1):
    ...
    # print(l.size())
    recon_l = model.decoder(sample)
    # print(recon_l.size())
    recon_loss = loss(recon_l, x)
    recon_loss = recon_loss.reshape(x.shape[0], -1).sum(dim = 1).mean()

    kl_loss = -0.5 * torch.sum(1 + var - mean**2 - var.exp(), dim = -1)
