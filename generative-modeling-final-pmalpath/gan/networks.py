...
class DownSampleConv2D(jit.ScriptModule):
    ...
    def __init__(
        ...
        self.downsample =  nn.PixelUnshuffle(downscale_ratio)
        self.conv = nn.Conv2d(input_channels, n_filters, kernel_size, padding=padding)
    ...
    def forward(self, x):
        ...
        x = self.downsample(x)
        # x = x.split(self.downscale_ratio**2, dim = 1)
        # print("Before split:", x.size())
        x = torch.chunk(x, int(self.downscale_ratio**2), dim = 1)
        x = torch.stack(x, dim = 0)
        x = torch.mean(x, dim = 0)
        # x = torch.div(x,4)
        # print("After split:", len(x))
        # for x_l in x:
        #     print("After split:", x_l.size())
        # x = torch.mean(torch.stack(x, dim = 0), dim = 1)
        return self.conv(x)
