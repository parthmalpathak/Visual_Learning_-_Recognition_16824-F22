...
def get_optimizers_and_schedulers(gen, disc):
    ...
    scheduler_discriminator = torch.optim.lr_scheduler.LambdaLR(optim_discriminator, lambda epoch : 1 - (epoch/500000), last_epoch = -1)
    optim_generator = torch.optim.Adam(gen.parameters() ,lr=0.0002, betas= (0, 0.9))
    scheduler_generator = torch.optim.lr_scheduler.LambdaLR(optim_discriminator, lambda epoch : 1 - (epoch/100000), last_epoch = -1)
    return (
        optim_discriminator,
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast():
                ...
                # print(train_batch[0])
                # n_samples = train_batch[0]
                generated_data = gen.forward(n_samples = train_batch.size()[0])
                ...
                eps = torch.randn((train_batch.size()[0],1,1,1), device = torch.device('cuda'))
                interp = eps * generated_data + (1 - eps) * train_batch
            ...
            #         # TODO 1.2: Compute samples and evaluate under discriminator.
                    discrim_fake = disc(gen.forward(n_samples = train_batch.size()[0]))
            ...
            if iters % log_period == 0 and iters != 0:
                with torch.no_grad():
                    with torch.cuda.amp.autocast():
                        ...
                        generated_samples = gen.forward(n_samples = train_batch.size()[0])
