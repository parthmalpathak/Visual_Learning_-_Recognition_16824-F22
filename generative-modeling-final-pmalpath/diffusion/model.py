...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        self.alphas_cumprod = torch.tensor(np.cumprod(alphas.cpu())).to(self.device)
        self.alphas_cumprod_prev = torch.tensor(np.append(1., self.alphas_cumprod[:-1].cpu())).to(self.device)
    ...
    def _get_posterior_variance(self):
        ...
        sigmas = ((1.0 - self.alphas_cumprod_prev)/(1.0 - self.alphas_cumprod))* self.betas
        return sigmas
        # raise NotImplementedError()

    def predict_start_image_from_noise(self, x_t, t, noise):
        # TODO (Q3.1): given a noised image x_t and noise tensor, predict x_0
        x_start = extract(torch.sqrt(1.0 / self.alphas_cumprod), t, x_t.shape)*(x_t - extract(torch.sqrt(1 - self.alphas_cumprod), t, x_t.shape)* noise)
        return x_start
    ...
    def predict_denoised_at_prev_timestep(self, x, t: int):
        ...
        time = torch.full((x.shape[0],), t, dtype = torch.long).to(self.device)
        noise = torch.randn_like(x).cuda() if t > 0 else 0. # no noise if t == 0
        model_mean, posterior_variance, posterior_log_variance, x_start = self.mean_variance_at_previous_timestep(x, time)
        pred_img = model_mean + torch.exp(0.5 * posterior_log_variance)*noise
    ...
    def ddpm_sample(self, shape):
        ...
        img = torch.randn(size = shape).to(self.device)
        for t in range(self.num_timesteps):
            img, x_start = self.predict_denoised_at_prev_timestep(img, (self.num_timesteps- t - 1))
            img = img.float()
