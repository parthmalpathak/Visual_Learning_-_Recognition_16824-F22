...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        boxScores = self.roi_data['boxScores'][0][index][:, 0]
        boxScores_sorted = np.argsort(-boxScores)
        boxScores_sorted = boxScores_sorted[:self.top_n]
        final_boxes = boxes[boxScores_sorted]

        normal = np.array([width, height, width, height])

        proposals = final_boxes/normal
...
def collate_fn(data):
    # data["image"] =  torch.nn.utils.rnn.pad_sequence(data["image"], batch_first = True)
    # data["label"] =  torch.nn.utils.rnn.pad_sequence(data["label"], batch_first = True)
    # data["wgt"] =  torch.nn.utils.rnn.pad_sequence(data["wgt"], batch_first = True)
    # data["rois"] =  torch.nn.utils.rnn.pad_sequence(data["rois"], batch_first = True)
    # data["gt_boxes"] =  torch.nn.utils.rnn.pad_sequence(data["gt_boxes"], batch_first = True)
    # data["gt_classes"] =  torch.nn.utils.rnn.pad_sequence(data["gt_classes"], batch_first = True)
    return (
        torch.stack([d["image"] for d in data]),
        torch.stack([d["label"] for d in data]),
        torch.stack([d["wgt"] for d in data]),
        [d["rois"] for d in data],
        [d["gt_boxes"] for d in data],
        [d["gt_classes"] for d in data]
    )