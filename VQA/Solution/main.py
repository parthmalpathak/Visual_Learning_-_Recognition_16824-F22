...
class Trainer:
    ...
    def __init__(self, model, data_loaders, args):
        ...
        self.writer = SummaryWriter('runs_extra_credit_3/' + args.tensorboard_dir)
    ...
    def train_test_loop(self, mode='train', epoch=1000):
        n_correct, n_samples, histogram_val = 0, 0, 0
        for step, data in tqdm(enumerate(self.data_loaders[mode])):
            ...
            loss = torch.nn.BCEWithLogitsLoss()(scores, answers)
            ...
            # Accuracy
            # print(scores)
            # print("This is the size:", scores.size())
            histogram_val += F.one_hot(scores.argmax(1), scores.size(1)).sum(0)
            # print(histogram_val.size())
            ...
            if mode == 'val' and step == 146:  # change this to show other images
                _n_show = 100  # how many images to plot
                for i in range(_n_show):
                    self.writer.add_image(
                        'Image%d' % i, data['orig_img'][i].cpu().numpy(),
                        epoch * _n_show + i, dataformats='CHW'
                    )
                    self.writer.add_text('Original Question %d' % i, 
                    data['question'][i], 
                    epoch * _n_show + i)

                    correct_answer = torch.argmax(answers, dim = 1)[i].item()
                    # correct_answer = answers[i]
                    self.writer.add_text('Original Answer %d' % i, 
                    self._id2answer[correct_answer], 
                    epoch * _n_show + i)
                    
                    predicted_answer = torch.argmax(scores, dim = 1)[i].item()
                    self.writer.add_text('Predicted Answer %d' % i, 
                    self._id2answer[predicted_answer], 
                    epoch * _n_show + i)
            ...
            self.writer.add_scalar(
                'Accuracy/' + mode, n_correct/n_samples,
                epoch * len(self.data_loaders[mode]) + step
            )
        y_val, indices = torch.sort(histogram_val, descending = True)
        # print(indices)
        indices = indices.tolist()
        # print(indices)
        for i in range(10):
            print("{} class is: {}".format(i + 1, self._id2answer[indices[i]]))
        # print(y_val)
        y_val = y_val.cpu().detach()
        # y_val = y_val[torch.nonzero(y_val)]
        y_val = y_val.tolist()
        # print(len(y_val))
        # y_val = histogram_val.values()
        # hist = torch.histc(histogram_val)
        x_val = np.arange(0, 5217).tolist()
        # plt.hist(y_val, bins = 5217)
        plt.bar(x= x_val[:400], height= y_val[:400], width= 3)
