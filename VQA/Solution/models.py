...
class BaselineNet(nn.Module):
    ...
    def __init__(self, n_answers=5217):
        ...
        for param in self.vis_encoder.parameters():
                    param.requires_grad = False
        # for child in self.vis_encoder.children():
        #     cntr += 1
        #     if cntr < 6:
        # # print(cntr)
        #         for param in child.parameters():
        #             param.requires_grad = False
        # for param in self.vis_encoder.parameters()[-5:]:
    ...
    def compute_vis_feats(self, image):
        ...
        x = self.vis_encoder(image)
        x = x.mean([2, 3])
        return x  # feed to vis_encoder and then mean pool on spatial dims
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, seq1, seq1_key_padding_mask, seq2,
        ...
        seq1 = self.norm_12(seq1 + self.dropout_12(cross1b))
        
        ffn12 = self.ffn_12(seq1)
        seq1 = self.norm_122(seq1 + ffn12)
        ...
        seq2 = self.norm_21(seq2 + self.dropout_21(cross2b))

        ffn21 = self.ffn_21(seq2)
        seq2 = self.norm_212(seq2 + ffn21)
